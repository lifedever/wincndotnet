...
var tlRobot = require('./tl.robot');

var articleService = require('../service/article.service');
var wxService = require('../service/wx.service.js');
...
var _replay = function (wx, res) {
    wxService.findOneRobot({key: wx.Content.toLowerCase()}, function (err, doc) {
        if (doc) {                  // 查询系统预置的规则
            res.reply({
                content: doc.value,
                type: 'text'
            ...
            return;
        } else if (lodash.startsWith(wx.Content, '@')) {   // 查询文章
            wx.Content = lodash.trim(wx.Content);
            if (wx.Content.length < 1) {
                res.reply({
                    content: '无结果返回，请重新查询!',
                    type: 'text'
                });
            ...
            articleService.search({title: new RegExp(keyword, 'i')}, function (err, arts) {
                if (err) {
                    res.reply({
                        content: err,
                        type: 'text'
                    ...
                    for (var i = 0; i < arts.length; i++) {
                        ...
                        var art = {
                            ...
                            picurl: 'http://gravatar.duoshuo.com/avatar/c1cb3c9970aa9f02e3ba067013af8c6c?s=100&r=G&d=retro',
                            url: url
                        };
                        artsList.push(art);
                    }
